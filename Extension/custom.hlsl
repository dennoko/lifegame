...
// Inserting a process into pixel shader
float4 GetLifeGameColor(float2 uv)
{
    // 1. Calculate Grid Coordinates
    float2 gridUV = uv * _GridResolution;
    ...
    float state = tex2D(_SimulationTex, samplePos).r;
    ...
    float4 finalColor = _DeadColor;
    finalColor = lerp(finalColor, _AliveColor, isAlive * shapeAlpha);

    return finalColor;
}

#define BEFORE_LIGHTING \
    float4 lgColor = GetLifeGameColor(fd.uvMain); \
    fd.albedo = lgColor.rgb;
//#define OVERRIDE_xx
